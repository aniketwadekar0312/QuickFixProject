...
exports.createReview = async (req, res) => {
  try {
    ...
    if (req.user.role !== "customer") {
      return res
        .status(403)
        .json({ status: false, message: "Only customers can submit reviews" });
    }
    ...
    if (booking.status !== "completed") {
      return res
        .status(400)
        .json({ status: false, message: "Can only review completed bookings" });
    }
...
exports.getCustomerReviews = async (req, res) => {
  try {
    ...
    const reviews = await Review.find({ customer: req.user.id })
      .populate("worker", "name")
      .populate("customer", "name email photoUrl")
      .populate({
        path: "booking",
        populate: {
          path: "service",
...
exports.getAllReviews = async (req, res) => {
  try {
    const reviews = await Review.find({ rating: { $gte: 4 } })
      .populate({
        path: "customer",
        select: "name email photoUrl", // Add relevant fields
      })
      .populate({
        path: "booking",
        select: "service",
        populate: {
          path: "service",
          select: "name", // Selecting only the service name
        },
      });
   ...
   return res.status(200).json({
    ...
    return res.status(500).json({ status: false, message: "Review not found" });
  }
};
